...
class MyAgent(AlphaBetaAgent):
    ...
    def evaluate(self, state):
        ...
        for pawn in range(5):
            ...
            # Idea: a "1 pawn advancement" should advance if it is safe
    ...
    def pawn_2_4_returning_safe_advance_yellow(self, state, pawn):
        """
        Gives additional points if the pawn 2 of the player yellow is in the returning path
        and is safe to advance
        :param state: the state
        :return: the additional heuristic value
        """
        if not state.is_pawn_returning(self.id, pawn) or state.is_pawn_finished(self.id, pawn):
            return 0
        s = 0

        my_advancement = state.get_pawn_advancement(self.id, pawn) % 6

        if (state.get_pawn_advancement(1 - self.id, my_advancement) % 6) + \
                squadro_state.MOVES_RETURN[1 - self.id][my_advancement] < pawn:
            s += 5
        return s
